	...
	execute: async (client, message, args) => {
		...
		function generateGame(word) {
			...
			setTimeout(() => {
				...
				message.channel.send(`Kata baru: ${word}`);
				...
				collector.on("collect", (msg) => {
					...
					if (msg.content.toLowerCase().indexOf(word) >= 0 && wordList.map((word) => word.toLowerCase()).indexOf(msg.content.toLowerCase()) >= 0) {
						...
						return msg.channel.send(`KATA TIDAK VALID \n By: ${msg.author.toString()}`);
					}
				});

				collector.on("end", async (collected, reason) => {
					if (reason === "time") {
						return message.channel.send("TIDAK DITEMUKAN PEMENANG");
					} else {
						message.channel.send(`Kata ini ditemukan oleh <@${reason}>`);
						winners.push(reason);
					}
					if (i < nbGames - 1) {
						i++;
						...
						if (winners.length < 1) {
							return message.channel.send("ADA LEBIH DARI 2 PEMENANG");
						}
						const winnerID = await getWinner(winners);
						const time = createdAt;
						const user = await client.users.fetch(winnerID);

						message.channel.send(`Game Berakhir Statistik game adalah - \n Pemenang: ${user.username}\n jumlah peserta: ${participants.length}\n daftar peserta: ${participants.map((p) => "<@" + p + ">").join("\n")}`);
